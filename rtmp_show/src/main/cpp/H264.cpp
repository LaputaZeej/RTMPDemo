...
int RTMP264_Connect(const char *url) {
    LOG("RTMP264_Connect");
    // 1
    LOG("RTMP264_Connect-1 init");
    nalhead_pos = 0;
    m_nFileBufSize = BUFFER_SIZE;
    m_pFileBuf = (unsigned char *) malloc(BUFFER_SIZE);
    m_pFileBuf_tmp = (unsigned char *) malloc(BUFFER_SIZE);
    // 2
    LOG("RTMP264_Connect-2 init rtmp");
    m_pRtmp = RTMP_Alloc();
    RTMP_Init(m_pRtmp);
    if (RTMP_SetupURL(m_pRtmp, (char *) url) == FALSE) {
        LOG("RTMP264_Connect-2-0 RTMP_SetupURL fail");
        RTMP_Free(m_pRtmp);
        return FALSE;
    }
    RTMP_EnableWrite(m_pRtmp);
    if (RTMP_Connect(m_pRtmp, 0) == FALSE) {
        LOG("RTMP264_Connect-2-1 RTMP_Connect fail");
        RTMP_Free(m_pRtmp);
        return FALSE;
    }

    if (RTMP_ConnectStream(m_pRtmp, 0) == FALSE) {
        LOG("RTMP264_Connect-2-2 RTMP_ConnectStream fail");
        RTMP_Close(m_pRtmp);
        RTMP_Free(m_pRtmp);
        return FALSE;
    }
 ...
 * @成功则返回1 , 失败则返回0
 */
int RTMP264_Send(int (*read_buffer)(unsigned char *buf, int size)) {
    ...
    ret=read_buffer(m_pFileBuf,m_nFileBufSize);
    if(ret<0)
    {
        LOG("RTMP264_Send fail");
        return FALSE;
    }
