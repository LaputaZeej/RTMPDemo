...
float fs(float r, float rmax, vec2 scale){
    return (1.0-pow(r/rmax-1.0, 2.0)*scale.xy.x);
}

vec2 newCoord(vec2 coord, vec2 eye, float rmax, vec2 scale){
    vec2 p = coord;
...
void main(){
    ...
    vec2 p = newCoord(aCoord, left_eye, rmax, scale);
    p = newCoord(p, right_eye, rmax, scale);
